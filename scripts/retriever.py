...
def get_vectorstore() -> FAISS:
  return FAISS.load_local(str(VSDIR), emb,
...
def filtered_similar_docs(query: str, where: Dict[str, Any] | None = None, k:
  int = 6):
  """Apply a simple post-filter over metadata keys.
   ...
   """
  vs = get_vectorstore()
  docs = vs.similarity_search(preprocess_query(query), k=25) # grab more,
  then filter
  if where:
    kept: List = []
  for d in docs:
